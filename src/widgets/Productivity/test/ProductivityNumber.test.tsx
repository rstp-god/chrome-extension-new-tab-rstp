...
describe('ProductivityNumber', () => {
  ...
  it('baseline=null, coldStart=false → no delta number or comparison text shown', () => {
    ...
    expect(screen.queryByTestId('delta-positive')).toBeNull()
    expect(screen.queryByTestId('delta-negative')).toBeNull()
    expect(screen.queryByTestId('delta-as-usual')).toBeNull()
    expect(screen.queryByTestId('delta-cold-start')).toBeNull()
  ...
  it('coldStart=true → cold-start element is shown; no delta number', () => {
    ...
    const coldStartEl = screen.getByTestId('delta-cold-start')
  ...
  it('value > baseline → +N delta with comparison label and positive color class', () => {
    ...
    const el = screen.getByTestId('delta-positive')
  ...
  it('value === baseline → "as usual" text is shown', () => {
    ...
    const el = screen.getByTestId('delta-as-usual')
    expect(el).toBeDefined()
