...
2. Pick the relevant baseline median for today's day type (weekday or weekend, depending on `splitWeekdayWeekend`). If either `closedBaseline` or `wipBaseline` is `null` → **cold**.
