...
describe('useAppearanceStore', () => {
  ...
  it('setGridPreset switches preset values and rescales widgets when columns change', async () => {
    await useAppearanceStore.getState().setGridPreset('spacious')
  ...
  it('setGridPreset awaits widget commit BEFORE appearance setState (ordering guarantee)', async () => {
    const order: string[] = []
    commitMock.mockImplementationOnce(async () => {
      order.push('widget-commit')
    })
    const unsubscribe = useAppearanceStore.subscribe(() => order.push('appearance-setstate'))

    await useAppearanceStore.getState().setGridPreset('spacious')
    unsubscribe()

    // widget store commit must land before the appearance store emits its
    // grid update — otherwise a mid-op crash would leave the two envelopes
    // desynchronised.
    expect(order).toEqual(['widget-commit', 'appearance-setstate'])
  })

  it('setGridPreset to custom without columns change does not rescale', async () => {
    await useAppearanceStore.getState().setGridPreset('custom')
    expect(setWidgetsMock).not.toHaveBeenCalled()
    expect(useAppearanceStore.getState().grid.preset).toBe('custom')
    // columns preserved from previous preset
    expect(useAppearanceStore.getState().grid.columns).toBe(DEFAULT_APPEARANCE.grid.columns)
  })

  it('setGridPreset("custom") is a no-op when already custom', async () => {
    await useAppearanceStore.getState().setGridPreset('custom')
    setWidgetsMock.mockClear()
    await useAppearanceStore.getState().setGridPreset('custom')
    expect(setWidgetsMock).not.toHaveBeenCalled()
  ...
  it('setGridCustom with columns triggers rescale', async () => {
    await useAppearanceStore.getState().setGridCustom({ columns: 24 })
    expect(setWidgetsMock).toHaveBeenCalledTimes(1)
    expect(useAppearanceStore.getState().grid.preset).toBe('custom')
    expect(useAppearanceStore.getState().grid.columns).toBe(24)
  })

  it('setGridCustom without columns does not rescale', async () => {
    await useAppearanceStore.getState().setGridCustom({ gap: 20 })
    expect(setWidgetsMock).not.toHaveBeenCalled()
  ...
  it('resetToDefaults restores defaults', async () => {
    ...
    await useAppearanceStore.getState().resetToDefaults()
    const state = useAppearanceStore.getState()
